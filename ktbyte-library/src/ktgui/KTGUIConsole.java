...
public class KTGUIConsole extends Controller {
	...
	public KTGUIConsole(KTGUI ktgui, String title, int posx, int posy, int w, int h) {
		...
		inputBox = new InputTextBox(ktgui, "ConsoleTextBox", posx, posy + h - inputBoxWidth, w, inputBoxWidth);
		...
		textArea = new ScrollableTextArea(ktgui, "sta:" + title, posx, posy, w - inputBoxWidth, h - inputBoxWidth);
		textArea.setBorderRoundings(BOX_ROUNDING, 0, 0, 0);
		//attachController(textArea);
		//registerChildController(textArea);
		
		scrollBar = new ScrollBar(ktgui, "scrollbar:" + title, posx + w - inputBoxWidth, posy, inputBoxWidth, h - inputBoxWidth, 0, 100);
