...
class Panel extends Controller {
  ...
  Panel(int posx, int posy, int w, int h) {
    this.posx = posx;
    this.posy = posy;
    this.w = w;
    this.h = h;
  ...
  void attachController(Controller controller) {
    if (controller.parentPanel != null) {
      controller.parentPanel.controllers.remove(controller); // remove 'this' controller from parentPanel's children-controllers list
    }

    if (!controllers.contains(controller)) {
      controllers.add(controller);
      controller.setParentPanel(this);
    }
  }
